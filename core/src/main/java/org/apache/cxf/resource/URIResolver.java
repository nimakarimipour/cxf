...
public class URIResolver implements AutoCloseable {
    ...
    private void tryFileSystem(@RUntainted String baseUriStr, @RUntainted String uriStr) throws IOException, MalformedURLException {
        ...
        @RUntainted String fileStr = uriStr.replace("%20", " ");
        ...
        try {
            ...
            if (relative.isAbsolute()) {
                ...
                try {
                    @RUntainted HttpURLConnection huc = createInputStream();
    ...
    private void tryArchive(@RUntainted String baseStr, @RUntainted String uriStr) throws IOException {
        ...
        @RUntainted String archiveBase = baseStr.substring(0, i + 1);
        String archiveEntry = baseStr.substring(i + 1);
        try {
            @RUntainted URI u = new URI(archiveEntry).resolve(uriStr);
